...
type proxyHandler struct {
	...
	// ja4Fingerprint is the JA4 TLS fingerprint of the client (if available).
...
func (h proxyHandler) ServeHTTPAuthenticated(w http.ResponseWriter, r *http.Request, client, authUser string) {
	...
	if h.ja4Fingerprint != "" {
		r = r.WithContext(context.WithValue(r.Context(), ja4FingerprintKey{}, h.ja4Fingerprint))
	}
	...
	acceptEncoding := parseAccept(r.Header, "Accept-Encoding")
	filteredEncodings := make([]acceptSpec, 0, len(acceptEncoding))
...
func (resp *Response) SetContent(data []byte, contentType string) {
	...
	if len(data) > 1000 {
		encoding := negotiateContentEncoding(resp.Request.Request, []string{"br", "gzip"})
